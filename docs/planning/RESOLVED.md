...
- **`POST /communications/sms` had no `ENABLE_SMS` gate.** With SMS globally off pending 10DLC, Telnyx accepts the send and reports success anyway (error 40010 at the carrier), so this route answered `{ success: true, messageId }` for a text that never arrives. Now 503 (nothing sent) unless `ENABLE_SMS` is the literal `'true'`; auth (401) and body validation (400) unchanged. The dashboard never calls the route. Test: `tests/routes/communications.smsGate.test.ts` (6 red on old code, all green after). **Known residual, not fixed here:** `grep ENABLE_SMS src/` shows this route and `/appointments/:id/send-self-service-links` are the only two gated backend paths — the reminder worker and confirmation sends go through `CommunicationService`/`SMSService` with no gate (tracked as its own item).
