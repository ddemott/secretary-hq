...
- **Dependency security patches — root + dashboard now at 0 `npm audit` findings.** Baseline before: root 10 (1 critical, 4 high, 5 moderate), dashboard 8 (1 critical, 3 high, 4 moderate). **`next` 16.3.1 → 16.3.5 in the dashboard** (critical: two unauthenticated RCEs fixed in 16.3.3 — one affects Windows-hosted servers only, we deploy on Linux; the other is in the Image Optimization API "when AVIF files are used", and this app sets no `images` config, imports no `next/image` and ships no `.avif`, so exposure was likely nil — patched anyway as a one-patch bump). **`nodemailer` 9.0.5 → 9.1.1** in the backend (4 advisories: a quadratic-time address-parser DoS, and three recipient-domain / file-access bypasses; we send mail to user-supplied addresses); the `package.json` floor was raised to `^9.1.1` so a regenerated lockfile cannot resolve the vulnerable range. **Removed the backend's orphan `next` dependency** — nothing in `src/`, `scripts/`, `tests/`, configs or CI imports it and nothing depends on it (the dashboard has its own); it was flagged critical in the backend audit and dragged `sharp` into the backend install. Transitive `fast-uri`, `brace-expansion`, `qs`, `browserslist` and `vitest`/`@vitest/mocker` (a dev-only path-traversal in the mock redirect) were bumped via non-forced `npm audit fix` / explicit patch installs. Verified on the branch: backend `tsc` + eslint clean, **3,365 backend tests pass (281 files)**, dashboard `tsc` clean, **1,242 dashboard tests pass (113 files)**, and a real `next build --webpack` production build succeeds on 16.3.5; CI (incl. Playwright, which builds the dashboard) is the further gate. **Deliberately NOT touched: the `agent/` package** — see the open finding in `docs/planning/TODO.md`.
- **VoiceCallsView owner-judgment UX pass (recovered from an abandoned 2026-09-16 branch, verified, tested, and one real bug fixed).** Same recovery as the BusinessSettingsView entry just below: every claim checked against the code and the tests red-proven against the original components (exact tallies in the test-accounting sentence below). What it fixes: a failed call-history fetch was swallowed into an empty list, so the panel showed "No call history yet" to a tenant that has calls (now a distinct `role="alert"` error, shown only for a user-visible fetch — the silent 10s poll stays quiet); `MessagesInbox` had the same error/empty conflation once its toast faded (now split); its filter tabs gain `aria-pressed` + `type="button"` and both Refresh buttons gain `aria-label`; loading indicators become live regions. **Found and fixed during verification:** the branch's error handling was incomplete — a failed "Load more" (page 2+) still ran `setCallHistory([])` but set no error, so it wiped every loaded call and fell through to the very "No call history yet" copy the pass exists to remove. Now the loaded rows are kept, a `role="alert"` "Could not load more calls" appears beside the still-enabled button, and a retry clears it. The initial spinner also became a `role="status"` region with screen-reader text (an icon-only spinner has nothing for a live region to announce). Two more bugs in that same load-more/error handling were caught in review of PR #540 and fixed: Refresh cleared `historyError` but not `loadMoreError`, so after a failed "Load more" a good Refresh left a stale red banner over a freshly reloaded list; and the silent 10s poll could succeed after a failed first load but never cleared `historyError`, and because the error branch renders before the list the panel stayed stuck on the error while the data had arrived (now any successful first-page response, silent or not, clears it — including an honestly EMPTY response, which then shows the normal empty state). Two of my own first drafts of the regression tests for these passed against the buggy code (one because a `waitFor(no alert)` ran while the list was replaced by the loading spinner, one because a `has_more:false` refresh response hid the stale banner) — both were rewritten until they failed on the old code. Test accounting (measured, `dashboard/components/voice`): the branch shipped 7 tests of its own (3 in `VoiceCallsView.test.tsx`, 4 in `MessagesInbox.test.tsx`); against `main`'s ORIGINAL components 5 of them fail and 2 do not — the two "Refresh button has an accessible name" tests pass even without the `aria-label`, because testing-library also derives an accessible name from `title`, so that change was unpinned until now. Added 15 tests: `VoiceCallsView.test.tsx` (load-more failure keeps the rows, retry clears the error and appends, quiet background poll, Refresh clears the error, spinner announcement, explicit `aria-label`, Refresh clears the load-more banner, poll recovers after a failed first load, empty poll clears the error) and `MessagesInbox.test.tsx` (honest-empty raises no alert, error clears on refresh, one-list failure still reported, explicit `aria-label`, `aria-pressed` follows the selection, tabs are `type=button`). 13 of the 15 fail against the original components; the other 2 (quiet poll, honest-empty) are regression guards that pass either way. Against the branch AS FOUND (its original commit `74c170a4`) 6 fail — load-more failure, its retry, the spinner text, Refresh-clears-banner, poll-recovers, empty-poll — which is what proves those bugs were real. Whole directory: 77 pass with the fixes, 18 fail against the original components (5 branch-own + 13 added). Removed `VoiceCallsView` from the TODO "Un-audited surfaces" list.
- **BusinessSettingsView owner-judgment UX pass (recovered from an abandoned 2026-09-16 branch, then tested).** The branch had sat local-only and un-PR'd; before shipping it every claim was checked against the code and the tests were red-proven against `main`'s old components. What it fixes: the page-level "Loading settings..." gate was keyed on `teamSize === null`, which is ALSO the value after a rejected fetch, so a failed tenant-config load left the ENTIRE settings page stuck on that screen forever (now keyed on an explicit `configLoaded` flag set in success and catch); `BusinessTypeSection` and `ChecklistPresetSection` rendered a failed initial `getConfig()` as the same "Not set" / derived-default an honestly-unset tenant shows (now a distinct alert, picker disabled); `CalendarSyncCard` swallowed a failed status check into `console.error` and showed a connected owner the "not connected" buttons (now a visible alert); loading text became `role="status"` live regions. The branch's own 6 tests were 6-red against the old code; added 19 more so every changed behavior is pinned: three new files (`MyAvailabilityCard`, `CalendarSyncCard`, `BusinessTypeSection` had none) covering the loading / empty / loaded / FAILED split — including the negative cases (an honest "not connected" / "Not set" must NOT raise the error alert) and that an error clears once a later check succeeds — plus two `ChecklistPresetSection` assertions. 7 of those 19 fail against the old components (the run showed 8 failures; the eighth was the branch's own checklist test) and the rest are regression guards that pass either way. Known, deliberate limit: on a failed calendar check the connect buttons stay visible under the alert (the alert says to refresh); hiding them when state is unknown would be a behavior change beyond this pass. Removed `BusinessSettingsView` from the TODO "Un-audited surfaces" list. Two review findings on the PR were also fixed with red-proven tests: `configLoaded` was never reset, so a super-admin switching tenants saw the PREVIOUS tenant's settings while the new one loaded, and a slow response from the tenant just left could overwrite the current one (now reset per tenant + stale responses ignored); and on a failed load `ChecklistPresetSection` still showed a real-looking derived preset name and live toggles beside the error (now "Checklist unavailable" with every toggle disabled). Total new tests: 22.
...
and logged, not fixed" list is superseded by the fixes archived here.
...
### P0 Telnyx REFER config — nothing to configure
...
### P2 — three backend suite warnings + pg@9 single-client overlap (also 2026-09-08 RESOLVED entry)
...
### Dashboard component subdirectory migration (DONE 2026-09-12)
...
### Wizard Phase B + abandoned-test-number reaper
...
### Dense-view decomposition — five shipped slices (remaining open work stays in TODO)
...
**Found on `main` the same day, fixed 2026-09-11** (archived under the 2026-09-14
doc-hygiene entry below / above in newest-first order): urgent message raise-only,
declined-meeting goodbye gate, wrong-node-id open-ids refusal, night-shift morning half,
and dashboard `book_appointment_atomic` STRICT skill-map parity.
