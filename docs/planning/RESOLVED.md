...
- **#565 — per-call overage billing.** New tiers solo/growth/professional = 30/100/300 included calls/month at $1.00/$0.75/$0.60 per overage call (`overagePerCallUsd`, `overageChargeUsd` on statements). New `UsageCapLevel` value `'overage'`: a paid plan past its allowance keeps answering calls (`blocked=false`) and racks up overage charges instead of refusing service. Only the free tier (no plan, or an unrecognized one — `PLAN_CAP_FREE`, default 50) still blocks. Stripe itself does not charge the overage yet — no products, no registered webhook, `sk_test` key in prod — the metering is computed and stored, not billed.
- **#566 — card required for the 14-day trial.** Checkout now sets `payment_method_collection: 'always'`. The 14-day trial is granted only when the Stripe customer has no PAST subscription (`isFirstSubscription`, `stripe.subscriptions.list({status: 'all'})`; a lookup failure fails closed — no trial, not a free one). `POST /provisioning/activate` returns 402 `subscription_required` unless `subscription_status='active'` (super-admin and deleted tenants exempt). Landing/register copy dropped "no credit card required" — trial is for new accounts, and a card is on file from the start.
- **#567 — signup email verification, one account per email, signup closed until launch.** New `email_verifications` table (48h single-use SHA-256 token); `POST /verify-email` (public) and `POST /verify-email/resend` (auth'd, 3/hr); `POST /billing/checkout` refuses 403 `email_not_verified` until `users.email_verified_at` is set. Reset-password and consent-confirm also verify the email. All existing prod users were backfilled verified. One account per email platform-wide, case-insensitive (`users_email_lower_unique` index, migration `20260924000200`) — checked in `/register`, admin `/tenants/create`, and `/users/invite`; this **deliberately reverses BUG-002**. `/demo/start` now issues `demo+<tenant_id>@quicklubedemo.invalid` addresses (27 old shared demo rows renamed) so demo signups don't collide under the new unique index. Self-serve signup is now CLOSED by default: `POST /register` returns 403 `signup_closed` unless `ENABLE_SIGNUP=true` (unset in prod); `GET /signup-status` reports the flag; `/register` shows "Sign-ups aren't open yet" when closed. **Bug found and fixed in the same PR:** `/register` 500'd for every templated business type under the RLS-enforced `app_user` role — the `on_tenant_created_resources` trigger's `resources` INSERT ran with no tenant context set. Migration `20260924000100` makes `create_default_resources` `SECURITY DEFINER`, scoped to `NEW.tenant_id` from the trigger row, not a caller-supplied value. Migrations `20260924000000`/`000100`/`000200` applied to prod before merge (210 migrations total).
- **#567 review fix** — `dashboard/lib/api.ts`'s `apiMutate` **resolves** `{success:false, error, ...}` on a non-2xx response; it only throws on a network failure. `BillingView` had been written as if a refused checkout threw, so the failure path redirected to Stripe with an `undefined` session id. Fixed to check `result.success`.
- **#568/#569/#570 — landing + billing catch up to the decided prices.** Monthly-only tiers $29.95 / $59.95 / $149.95 for 30/100/300 calls, annual toggle removed (no annual option, no discount). Per-tier staff/station limits removed from the plan cards — SecretaryHQ doesn't sell seats. Call transfer to a person listed as included on every plan (it already worked on every plan once a transfer number was set; the pricing page previously implied otherwise).
- **#571 — the agent remembers caller preferences, per business type.** `shared/preferenceCatalog.ts`: a universal preference set (preferred staff, days, time of day, contact method, language, notes) plus a list per business vertical (33 verticals covered; CI fails on one with no list). No medical keys anywhere (med_spa is appearance-only), law firm is contact-preferences-only. `tenants.tts_voice`-style tenant-config now sends `preference_catalog` to the agent. A host-built `remember_preference` tool is offered on every call (base tool, not tree-gated) whenever the owner's `save_preferences_enabled` toggle (default on) is on and the business has a non-empty catalog; its `key` argument is an enum of the catalog. The host holds preferences in call memory and only writes them via `save_customer_preference` to a profile this caller actually owns: the carrier caller-ID number for a recognized returning caller, a profile `identify_caller` saved this call, or a spoken number the caller proved with `verify_phone_code` — a spoken, unproven number gets nothing written (no SMS/10DLC to confirm it against). Saves happen in the background; `finish_call` makes one bounded 1.5s last attempt. **`save_customer_preference` is no longer "defined but unreachable"** — it is now reached through `remember_preference` (`agent/src/checklist/callerPreferences.test.ts`).
- **Doc pass (this entry):** `CLAUDE.md`, `docs/architecture/ARCHITECTURE.md`, `docs/planning/TEST_COVERAGE.md`, `docs/operations/DEPLOYMENT.md`, `docs/product/SECRETARYHQ_FEATURES.md`, `docs/operations/OWNER_GUIDE.md`, `docs/operations/SECURITY.md`, `docs/workflow/LESSONS_LEARNED.md`, and `docs/product/PRICING_RESEARCH.md` updated to match. Suite totals re-measured 2026-09-25 on `main` through #571: backend 3,477 / 288 files, dashboard 1,274 / 115, agent 1,107 / 65.

**Lessons (added to `docs/workflow/LESSONS_LEARNED.md`):** a test that mocks `apiMutate` to *throw* on failure proves the mock, not the code — `apiMutate` resolves `{success:false}` on a non-2xx response and only throws on a network error, so the mock had been asserting the wrong contract; `/register` had only ever been tested against a mocked `pg`, so the RLS 500 under the real `app_user` role was invisible until a real-DB test exercised it as a non-bypass role.
...
- [x] **[prod]** **`DASHBOARD_URL` defaults to `https://localhost:4400`** — FIXED 2026-06-16: boot warning now fires. **Prod action**: set `DASHBOARD_URL=<dashboard URL>` on Railway.
