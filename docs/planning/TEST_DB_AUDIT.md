...
**Created 2026-07-01** (test paths updated 2026-09-18: the backend tests moved from `src/` to `tests/`) (branch `test/blindspot-p0-verification`), fulfilling the
...
|---|---|---|---|
| `tests/services/serviceResolver.test.ts` | serviceResolver 3-branch SQL (the prod ambiguous-`name` 500) | `tests/integration/serviceResolver.realdb.test.ts` + `tests/routes/agentTools/agentToolsBookingIntegration.test.ts` (route-level regression) | ✅ pre-existing + extended this branch |
| `tests/routes/analytics.test.ts` | `/analytics/*` date-range SQL (`$n::date`, COALESCE-interval, FILTER, GROUP BY, abandonment_by_service) | `tests/integration/analytics.realdb.test.ts` | ✅ added this branch |
| `tests/routes/auditLog.test.ts` | dynamic WHERE + date casts + dynamic LIMIT/OFFSET indices | `tests/integration/auditLog.realdb.test.ts` | ✅ added this branch |
| `tests/routes/versionHistory.test.ts` | **interpolated identifiers** `${table}`/`${pkColumn}` + dynamic whereClause | `tests/integration/versionHistory.realdb.test.ts` | ✅ added this branch |
| `tests/routes/voice.test.ts` | dynamic whereClause/paramIndex + LIMIT/OFFSET + is_deleted filters | `tests/integration/voice.realdb.test.ts` | ✅ added this branch |
| `tests/services/reminders/scheduleForAppointment.test.ts` | dynamic multi-row INSERT into reminder_schedules | `tests/integration/scheduleForAppointment.realdb.test.ts` | ✅ added this branch |
| `tests/routes/agentTools/agentTools.test.ts` (customer search) | concatenated-column ILIKE name search | `tests/integration/agentToolsCustomerSearch.realdb.test.ts` | ✅ added this branch |
| `tests/routes/agentTools/agentTools.test.ts` (booking) | booking RPC calls + tz conversion | `tests/routes/agentTools/agentToolsBookingIntegration.test.ts` | ✅ added this branch |
| `tests/routes/appointments.test.ts` | dynamic date WHERE + dynamic UPDATE SET list | `appointment-date-filter.test.ts`, `appointment-mutations.test.ts` | ✅ pre-existing |
| `tests/routes/shifts-routes.test.ts` | `get_effective_shifts(_bulk)` RPC signatures + ::DATE casts | `shift-overrides-edge.test.ts` | ✅ pre-existing |
| `tests/routes/knowledge.explain.test.ts` | `search_tenant_docs_normalized` RPC (::vector) | `knowledge-policy-answer.test.ts` | ✅ pre-existing (RPC level) |
| `tests/routes/knowledge.importWebsite.test.ts` | ::vector INSERTs | `knowledge-import-document.test.ts` | ✅ pre-existing |
| `tests/services/expandWeeklyToSchedule.test.ts` | dynamic multi-row INSERT into employee_schedule | `expand-weekly-integration.test.ts` | ✅ pre-existing |
...
`tests/integration/agentToolsCancelReschedule.realdb.test.ts` drives the real
...
- `tests/integration/exportData.realdb.test.ts` — the strongest of the batch:
...
- `tests/integration/selfService.realdb.test.ts` — token-gated `/self/cancel` +
...
- `tests/integration/agentToolsMessages.realdb.test.ts` — `take-message` (customer_messages
...
- `tests/integration/reminders.deliveryStats.realdb.test.ts` — seeds a known set of
...
- `tests/integration/voiceSessionReaper.realdb.test.ts` — the `reap_stale_voice_sessions()`
...
- `tests/integration/users.realdb.test.ts` — auth-adjacent CRUD: GET /users (no
...
- `tests/integration/crmSync.realdb.test.ts` — getCrmSyncStatus (entity_sync_map
  ...
  clean zero shape).
- `tests/integration/skills.realdb.test.ts` — create slugifies ("Hair Coloring" →
  "hair-coloring"), list returns it, delete-by-slug removes it (the round-trip a
  name mismatch would strand), unknown delete → 404.
- `tests/integration/agentToolsAiCost.realdb.test.ts` — the **dynamic multi-row** INSERT
...
- `tests/integration/communications.realdb.test.ts` — GET /communications/history: the
