...
- [ ] **(code) NEW 2026-09-15 — Scheduler List/Day view silently drops evening appointments in UTC-behind timezones.** `dashboard/components/scheduler/useSchedulerData.ts` builds its date-range query as `` `${dateStr}T00:00:00Z` `` / `` `${toDateString(nextDay)}T00:00:00Z` ``, where `dateStr` is the **local** calendar date (`toDateString()` uses `getFullYear/getMonth/getDate`) but the query literally appends `Z` (UTC) to it — treating a local date string as if it were a UTC one. For a tenant in US Central (UTC-5 CDT), this shifts the fetch window 5 hours early: from roughly **19:00 local onward each day**, any appointment whose UTC timestamp has already rolled onto the next UTC calendar day falls outside the `[dateStr 00:00Z, nextDateStr 00:00Z)` range and never appears in the List/Day view for "today" — even though it's still today locally and the appointment exists correctly in the DB. Discovered as the root cause of a persistent (not flaky) `e2e/appointment-cancel-ui.spec.ts` CI failure on 2026-09-15 night (all failures landed 22:54–23:20 CDT, squarely in the broken window; every prior green CI run in the project's history happened to run outside 19:00–00:00 CDT, which is why this has never surfaced before). Confirmed via direct code read, not just timing correlation — this is a genuine 2-line bug in one function, not a test artifact. Not fixed here: this batch's PRs are unrelated UX/doc-hygiene work and this needs its own dedicated fix (construct the UTC boundary from local midnight correctly, e.g. via the existing `getTimezoneOffset()`-subtraction pattern already used correctly elsewhere in `NewSchedulerView.tsx` L649-650) plus regression coverage for the evening-crossing case specifically. Likely affects every consumer of `useSchedulerData` (Scheduler List + Day views) for any tenant not in a UTC-ahead timezone. **Worth a live-data check**: has this been silently hiding real evening bookings from owners in production since the feature shipped?
