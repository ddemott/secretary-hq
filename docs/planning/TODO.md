...
- [ ] **(code) NEW 2026-09-16 — HIGH: `/tenants/:id/update-config` (and `GET /tenants/:id/config`) have no server-side role check; any authenticated front_desk login can hijack live call transfers and overwrite attested legal disclosure text.** `src/routes/tenants.ts` (~lines 302-350): both routes gate only on `requireAuth` + tenant-match-or-super-admin (`req.auth?.tenant_id === id`, else `requireSuperAdmin`-equivalent check) — verified directly against the code, there is no `req.auth.role` check anywhere in either handler. `UpdateConfigSchema` (~lines 50-121) lets **any authenticated tenant user, owner or front_desk, no distinction**, write: `forward_phone` (the live call-transfer destination — every future "talk to a person" call routes wherever this points), `call_disclosure` + `disclosure_attested` (the owner's legally-attested caller-disclosure text, meant to affirm the owner's own state-law compliance — see the schema comment at line ~104), `checklist_preset_id` / `checklist_overrides` (booking_mode, message_mode, disabled conversation blocks — what the AI will and won't do on every call), `tts_voice`/style flags, `owner_phone`, `logo_url`, `business_type`.
  ...
  - Fix: add `role === 'owner'` gating (or equivalent) to `/tenants/:id/update-config` at minimum — `GET /tenants/:id/config` is lower urgency (a read, not a write) but should get the same review — and to the services/skills/mappings routes listed above, matching the existing pattern already used at `/customers/import` (`src/routes/customers.ts` ~line 230, `role === 'owner'` check with the comment "bulk PII writes are not a front-desk operation").
...
- [ ] Un-audited surfaces — **[REVIEW]** before beta (owner-judgment layout/flow; copy/a11y partials already landed). Most already had a copy/a11y **partial fix** 2026-07-03 plus correctness/a11y defect batch 2026-07-05 (PR #200). What remains is the owner-judgment call. `AIConfigView` (PR #492) and `LoginView` + `/forgot-password` + `/reset-password` (PR #499) got their owner-judgment UX pass 2026-09-14/16 — removed from this list. `SettingsView` (PR #501), `ProfileView` (PR #502), `DeletedRecordsPanel` + `RecordHistoryModal` (PR #500) have passes **open, not yet merged** as of 2026-09-16 — leave listed here until those land, then close out the same way:
  - `AnalyticsView`, `VoiceCallsView`, `AppointmentView` + `AppointmentDetailPanel` + `AppointmentListSidebar`, `CRMView` + `CustomerDetailPanel`, `ProfileView`, `BusinessSettingsView`, `SettingsView`, `EmployeeManagementView`, `ShiftManagementView`, `ResourceManagerView`, `ServiceAssignmentView`, `SkillMatrixView` + `SkillAssignmentsView` + `SkillRelationshipMap`, `DeletedRecordsPanel` + `RecordHistoryModal`, `/register`, `SuperAdminDashboard` + tenant cards/forms, `FirstRunTour`.
