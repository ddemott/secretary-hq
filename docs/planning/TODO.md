...
- [ ] **(Dale, use wife's phone)** **Live validation call** — do these steps together in one sitting:
  ...
  4. **Human-request failure-mode observation (until wire-up):** prefer the greeting word **"representative"** (also try once "talk to a person"). **Do not expect your cell to ring** — `transfer_call` is still absent from every `selectedTools()` branch (`agent/src/checklist/checklistTools.ts`; `docs/operations/RUNBOOK.md` §7c). Pass criteria until the item below lands: no dead air; no false "you're connected" / transferred claim; lands a message (urgent or normal) or a clear can't-transfer; Calls tab shows the transcript. Booking + urgent-message DoD for this live call still lives in `docs/planning/PRODUCT_ROADMAP.md` T-003 (prefer urgent-message path for product acceptance). After wire-up, replace this step with a real ring + `transferred` outcome check.
  ...
  6. Validate the booked time is what was said, not an earlier slot. `docs/planning/RESOLVED.md` (2026-07-04) logged a `[~]` **partial**: `book_with_scheduling_atomic` books the EARLIEST open slot ≥ `window_from`, so a caller-named "4:30" could book 4:00 — code mitigation shipped (tool description + prompt sharpened to set `window_from` to exactly the picked time), but it was never live-proved and no later TODO item picked it up. Confirm the agent both books AND confirms back the actual `booked_start`, not the caller's stated time restated blindly.
     (PSTN inbound itself already confirmed 2026-06-30; this closes the booking + human-request degrade + preference legs. Live transfer ring is a post-wire-up check.)
