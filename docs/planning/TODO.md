...
- [ ] **(code)** **Three noisy warnings in the backend suite — none suppressed, all still real.** PR #402 shipped `VITE_CONFIG_NATIVE_IGNORE_WARNING=true` on `npm test` claiming to silence the first; the variable does not exist (the real one is `VITE_CJS_IGNORE_WARNING`), it targeted a CJS-deprecation warning the current Vite no longer emits, and the warning that DOES print is a different one. Removed rather than renamed — a no-op env var in a script reads as a solved problem. What actually prints on a green run:
  1. `vite:dynamic-import-vars` on `tests/regression/type-safety.test.ts` — `import('../../src/routes/${mod}')` has no static file extension. Fix the import shape in the test, not the logger.
  2. `pg` `DeprecationWarning: Calling client.query() when the client is already executing a query` — a real overlapping-query pattern in test helpers, removed in pg@9. This one is a future break, not cosmetics.
  3. `ai_cost_model_unpriced` WARN rows for `gpt-4o-mini-tts`, `grok-tts`, `gpt-9-imaginary` — deliberate fixture models with no price entry. Expected output of the SAD path; leave, or assert on it so it stops reading as a defect.
