...
**But defining a tool does not put it in front of the model.** Under question trees, `selectedTools()` (`agent/src/checklist/checklistTools.ts`) rebuilds the toolset from the currently selected trees, and presents **4 always-on tools** (`set_purpose`, `record_answer`, `finish_call`, `answer_question`), **plus only the action and passthrough tools of the trees this call selected** (drawn from a pool of 9: 6 action tools and 3 passthroughs — the 13-tool figure is that whole pool, not what one call sees), plus **`remember_preference` whenever the owner has not switched preference-saving off and this business has a preference list** (true by default for almost every tenant, #571), plus **`transfer_call` whenever a forward number is configured** (`ALWAYS_ON_PASSTHROUGH_TOOLS` + `offerTransfer`, shipped #462), plus **3 identity tools whenever the `identity` tree is selected** (which is true on goal-bearing calls, so most real calls with a forward number see 18 rather than 17):

| Group                                          | Tools                                                                                                                                                                                                                            |
| ---------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Base (always)                                  | `set_purpose`, `record_answer`, `finish_call`, `answer_question` (wraps `get_company_policy_answer` — RAG under another name)                                                                                                    |
| Base (owner-toggle gated)                      | `remember_preference` (host wrapper over `save_customer_preference`; offered whenever `save_preferences_enabled` is on and tenant-config sends a non-empty `preference_catalog` — true for almost every tenant by default, #571) |
...
**`save_customer_preference` is no longer on that list (2026-09-25, #571).** It is reached through a host-built base tool, `remember_preference` — offered on every call, not tree-gated, whenever the owner's `save_preferences_enabled` toggle (default on) is on and tenant-config sends a non-empty `preference_catalog` (`shared/preferenceCatalog.ts`: a universal preference set plus one list per business vertical, all 33 verticals covered). The model calls `remember_preference` with a key from that catalog's enum the moment a caller mentions a lasting preference; the host holds it in call memory and only writes it through `save_customer_preference` to a profile this caller actually owns — the carrier caller-ID number for a recognized returning caller, a profile `identify_caller` saved this call, or a spoken number proven with `verify_phone_code` (an unproven spoken number gets nothing written). Saves are backgrounded; `finish_call` makes one bounded last attempt. `agent/src/tools/reachability.ts`'s comment on `save_customer_preference` ("not selected by any tree; kept, not deleted") predates this and is stale — the tool is reached, just not as a per-tree action node.
