...
> drift `docs/operations/DEPLOYMENT.md` carried the stale "38" figure until corrected in #515. Don't
...
- `tenants.tenant_id` is a UUID; every tenant-scoped table has a `tenant_id` column FK'd to it with `ON DELETE CASCADE`.
- `set_tenant_context(uuid)` sets a session-local GUC (`app.current_tenant_id`); `withTenantClient(tenantId, fn)` (built by `createWithTenantClient(pool)` in `src/database/index.ts`) wraps every tenant-scoped route in a checkout-set-fn-clear-release lifecycle.
- Every tenant-scoped table **declares** `ENABLE ROW LEVEL SECURITY` + `FORCE ROW LEVEL SECURITY` + a policy of the shape `tenant_id::text = current_setting('app.current_tenant_id', true)`. This line pinned that count at "29" for months; **it was never actually a count of tenant-scoped tables specifically** — `pg_tables ... rowsecurity` (42 as of 2026-09-16, see the banner above) counts every RLS-enabled table in `public`, tenant-scoped or not (`business_templates`, `password_resets`, and `appointment_sync_map` carry RLS too, on a different policy shape than the `tenant_id`-column one this bullet describes, verified 2026-09-16 by cross-referencing `information_schema.columns` against the RLS-enabled table list — no `tenant_id` column on any of the three). Re-verify the tenant-scoped subset specifically by that same cross-reference, not by trusting either pinned number. **They are enforced — see the banner above.** (`message_delivery_status` got its RLS + 2 policies in migration `20260724000050`; it had been enabled with ZERO policies in prod, i.e. deny-all, and functioned only because the app role bypassed. `schema_migrations` is the one deliberate exemption: no tenant dimension, and RLS on it deny-alls the migration runner under `app_user` — see migration `20260802000000`. NB the schema-alignment guard compares tables and columns, **not RLS flags**, so RLS drift between prod and `baseline.sql` is still something only a direct probe will catch.)
- The application middleware adds defense in depth: `tenantMiddleware` in `src/middleware/fastify-middleware.ts` rejects any request that supplies `?tenant_id=<other>` or `body.tenant_id=<other>` differing from the JWT's `tenant_id` (unless caller is super-admin). Closed cross-tenant override gap on 2026-05-06.
...
- 39 multi-tenant isolation probes (`tests/regression/multi-tenant-isolation.test.ts`) run on every CI build against real Postgres, exercising query-string override + body-FK injection + cross-tenant id under JWT-only + **unauthenticated `?tenant_id=` access (read/write/delete)** + admin-route gating + RLS configuration metadata.
...
**Control: `requireOwnerRole(req, reply)` (`src/middleware/fastify-middleware.ts`) — 401 if unauthenticated, 403 unless `req.auth.role === 'owner'`, bypassed for the platform super-admin tenant — called first in every destructive / bulk-PII / business-config handler.** The dashboard hides these tabs from `front_desk` logins (`OutlookLayout.tsx`'s `isFrontDeskOnly`), but that is client-side only; this guard is the server-side enforcement.

- Coverage as of 2026-09-18 (`grep -c "requireOwnerRole(req" src/routes/*.ts`): `tenants`, `services`, `skills`, `mappings`, `employees`, `shifts`, `resources`, `customers` (`POST /customers/import`, `DELETE /customers/:id`), `provisioning`, `billing` (checkout/portal), `knowledge`, `calendar`, `versionHistory`, `setup` (`/setup/commit`, `/setup/default-service`, `/setup/impact`), and the Square OAuth scaffold (`crmRouteScaffold.ts`). Reads are deliberately left ungated except where they expose config (e.g. `GET /tenants/:id/config`).
- **Closed 2026-09-16:** the systemic "no server-side role check" gap (staffing CRUD, tenant config, services/skills/mappings, provisioning, billing, knowledge, calendar, version history) shipped in PR #522; the back doors it missed (`/setup/*`, Square OAuth actions) in PR #523. Detail: `docs/planning/RESOLVED.md` (2026-09-16 entries).

**Gaps acknowledged:**

- The guard is opt-in per handler, not a global hook — a NEW mutating route that forgets `requireOwnerRole` is unprotected against a front-desk JWT, and #523 exists because #522 missed routes that reached the same tables by a different path. Re-run the grep above when adding a route; there is no test that fails on an ungated mutating route.
...
| -------------------------------------------------------------- | ----------------------------------------------- | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------- | ------------------------------------------------------- |
| Stripe `/billing/webhook`                                      | `stripe-signature`                              | Stripe v1 (constructEvent)                                                                               | `stripe.webhooks.constructEvent`                              | `tests/routes/webhook-signatures.test.ts`               |
| Square `/square/webhook`                                       | `x-square-hmacsha256-signature`                 | HMAC-SHA256 over `${notificationUrl}${body}`, base64                                                     | `squareClient.verifyWebhookSignature`                         | `tests/routes/webhook-signatures.test.ts`               |
| Telnyx `/communications/telnyx/status` (SMS delivery receipts) | `telnyx-signature-ed25519` + `telnyx-timestamp` | Ed25519 over `${timestamp}\|${rawBody}`, key `TELNYX_PUBLIC_KEY`                                         | `verifyTelnyxSignature` (`src/services/telnyxWebhookAuth.ts`) | `tests/routes/communications.test.ts`                   |
| Telnyx `/communications/telnyx/inbound` (inbound SMS)          | same                                            | same — fails closed (503) when `TELNYX_PUBLIC_KEY` is unset; `/status` accepts + logs unverified instead | same                                                          | `tests/integration/telnyxInboundWebhook.realdb.test.ts` |
| Telnyx voice (SIP, not HTTP)                                   | n/a                                             | n/a — SIP layer auth via SIP Connection ID                                                               | n/a                                                           | n/a                                                     |

> The HubSpot / Jobber / ServiceTitan CRM webhooks (and their HMAC verifiers) were removed 2026-06-12 along with those integrations. Stripe and Square are the remaining HMAC-verified HTTP webhooks; the Telnyx SMS webhooks use Ed25519 (Telnyx offers no HMAC).

**Critical correctness detail:** all HMAC verifications use `req.rawBody` (preserved by the global JSON content-type parser, `src/jsonContentTypeParser.ts`, registered in `src/index.ts`), NOT `JSON.stringify(req.body)`. Re-serializing through V8 doesn't byte-match the original payload (whitespace, key order, number formatting differ), so signature math fails deterministically. This was a bug from 2026-04-22 to 2026-05-09 in the HubSpot/Square/Jobber routes; fixed in commit `4c3205d`. (The HubSpot/Jobber routes were later removed entirely on 2026-06-12; the rule still applies to the surviving Stripe and Square webhooks.)
...
- No global denylist. A compromised token can't be revoked mid-window without a password change. Mitigation: admin can run `UPDATE users SET password_changed_at = NOW() WHERE user_id = '<user>'` to force-invalidate sessions without changing the password, but this has no UI surface today.
...
- Tests: `tests/routes/agentTools/agentTools.test.ts` pins missing-header / wrong-value / unset-AGENT_SECRET / shorter-provided-secret-no-crash / rotation-accepts-OLD / rotation-rejects-third-value.
