...
| `ENABLE_WEBSITE_RESCAN_SCHEDULER`                                  | `false` outside prod          | Periodic website-KB re-scan worker. Prod ON unless exact `false`. Cost caps: `WEBSITE_RESCAN_STALE_DAYS` (default 30), `WEBSITE_RESCAN_BATCH_SIZE` (default 5/tick), `WEBSITE_RESCAN_INTERVAL_MS` (default 24h). Stages suggestions only — never auto-publishes. Skip tick if no `OPENAI_API_KEY`. |
