...
This applies all 206 migrations in order (tracked in `schema_migrations`, so re-runs only apply new files). It does **not** seed data — seeding is the separate `scripts/seed-db.sh`. Prefer `DATABASE_URL="<prod-url>" bash scripts/setup-db.sh` over the argument form so the password isn't left in shell history (see `docs/operations/DEPLOYMENT_CHECKLIST.md` §3).
