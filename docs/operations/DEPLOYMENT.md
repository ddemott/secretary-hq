...
This applies all 210 migrations in order (tracked in `schema_migrations`, so re-runs only apply new files). It does **not** seed data — seeding is the separate `scripts/seed-db.sh`. Prefer `DATABASE_URL="<prod-url>" bash scripts/setup-db.sh` over the argument form so the password isn't left in shell history (see `docs/operations/DEPLOYMENT_CHECKLIST.md` §3).
...
This is the single source of truth for environment variables across all three deployable services. The "Verified against code 2026-05-05, unchanged since 2026-04-30" claim that used to sit here was stale — the table below documents vars added well after that date (`WEBSITE_RESCAN_*`, `SENTRY_*`, `BETTER_STACK_TOKEN`, `EMAIL_SEND_DEADLINE_MS`, `PLATFORM_ADMIN_EMAIL`, `SMTP_HOST`/`SMTP_PORT`, among others) — so it contradicted its own content. Each row's own inline dating is the source of truth for when that var was added or changed; there is no single as-of date for the whole table.
...
| `STRIPE_SOLO_PRICE_ID`                                   | Billing checkout                                      | Stripe price ID for the Solo tier — **$29.95/mo, 30 included calls, $1.00/call overage** (decided 2026-09-24, `docs/product/PRICING_RESEARCH.md`). No product/price exists in Stripe yet (`sk_test` key, no `webhook_endpoints` registered) — create it there before setting this var.                                                                                                                                                                                                                                                                                                                 |
| `STRIPE_GROWTH_PRICE_ID`                                 | Billing checkout                                      | Stripe price ID for the Growth tier — **$59.95/mo, 100 included calls, $0.75/call overage** (decided 2026-09-24). Not yet created in Stripe.                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `STRIPE_PRO_PRICE_ID`                                    | Billing checkout                                      | Stripe price ID for the Professional tier — **$149.95/mo, 300 included calls, $0.60/call overage** (decided 2026-09-25). Not yet created in Stripe. All three tiers are monthly only (no annual option) with no per-tier staff/station limit and call transfer included.                                                                                                                                                                                                                                                                                                                               |
| `DASHBOARD_URL`                                          | Stripe checkout + OAuth redirects                     | Public URL of the dashboard. Default `https://localhost:4400`. **Phase 13 blocker if not set in prod.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Signup switch     | `ENABLE_SIGNUP` — self-serve `POST /register` is **closed unless this is exactly `true`** (owner decision 2026-09-24: not open yet). Leave it **unset in production** until launch; local `.env` and CI E2E set it to `true`. Admin `POST /tenants/create` and `/demo/start` are unaffected.                         |
