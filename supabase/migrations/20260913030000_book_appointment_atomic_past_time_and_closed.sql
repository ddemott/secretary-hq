-- book_appointment_atomic (the dashboard manual-booking RPC) never enforced
-- two rules book_with_scheduling_atomic (the phone path) has had since
-- 20260909210000 / 20260903000000: no booking in the PAST, and no booking on
-- a day the business has marked CLOSED (blackout_dates). Checked directly
-- against the current function body — neither check has ever existed here,
-- in any migration that touched this function. A front-desk API caller (or
-- a bug in the dashboard UI) could book yesterday, or book a holiday the
-- owner explicitly closed for.
--
-- Fix: add both guards, same wording and same one-minute grace as the phone
-- path, checked right after customer resolution and before the STRICT
-- skill-map / assignment checks — a closed day or a past time is refused
-- before any staffing search runs, same ordering rule as
-- book_with_scheduling_atomic.
