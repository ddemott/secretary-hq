-- The morning half of a night shift could never be booked.
--
-- Found 2026-09-11 while testing 20260909210000, reproduced against a
-- rolled-back transaction: an employee scheduled 22:00->06:00 every day
-- (employee_schedule row dated the START day), a 02:00 slot the NEXT
-- morning -> EMPLOYEE_NOT_SCHEDULED; 23:00 the same evening books fine.
--
-- WHY: every shift-coverage clause joined employee_schedule on
-- `es.shift_date = v_shift_date`, where v_shift_date is the SLOT's own
-- local date. A night shift's row is stored under the date it STARTED
-- (the evening), so a 2 AM booking the next calendar day looked for a row
-- dated that next day and found nothing — the row that actually covers
-- 2 AM is dated YESTERDAY. shift_covers_booking() itself already knows how
-- to reason about a night shift's wrap (end < start); it was just never
-- being CALLED for the row that matters, because the JOIN excluded it
-- before shift_covers_booking ever ran.
--
-- FIX: shift_row_covers_booking() wraps shift_covers_booking() and adds
-- the missing case — a shift row dated YESTERDAY covers a booking on
-- TODAY only if it is itself a wrapping night shift (end < start) and
-- today's slot does not also wrap into a third day. Every JOIN/EXISTS that
-- used to test `es.shift_date = v_shift_date` now tests
-- `es.shift_date IN (v_shift_date, v_shift_date - 1)` and calls
-- shift_row_covers_booking(es.shift_date, v_shift_date, ...) instead of
-- shift_covers_booking(...) directly — six call sites in
-- book_with_scheduling_atomic (the skills branch, the no-skills
-- employee-naming step, the narrow skill-less shift guard's two clauses,
-- the legacy no-service-id skill-array diagnostic, and the STRICT
-- unlinked-employee diagnostic), plus availabilitySearch.ts's suggest-side
-- JOIN in the same commit — suggest and enforce must read the calendar the
-- same way, or the agent offers a slot the booking then refuses (the
-- 2026-07-17 midnight-wrap lesson, migration 20260718003000).
--
-- WHAT IS UNCHANGED: a DAY shift (end > start) never gains cross-day
-- coverage — the new ELSE-day branch requires end < start, so this only
-- ever opens the door for a genuine overnight shift's morning tail.
-- book_appointment_atomic (the dashboard path) is untouched: it validates
-- a single chosen resource/employee against ONE day's shift row by design
-- and has never modeled night shifts; that gap is pre-existing and out of
-- scope here.
--
-- No live tenant runs night shifts today (Thinking Hammer is day-only), so
-- this changes no current call's behavior — it closes a gap the next
-- overnight tenant would otherwise walk straight into.

CREATE OR REPLACE FUNCTION public.shift_row_covers_booking(
    p_row_shift_date DATE,
    p_target_date    DATE,
    p_shift_start    TIME,
    p_shift_end      TIME,
    p_slot_start     TIME,
...
COMMENT ON FUNCTION public.shift_row_covers_booking(DATE, DATE, TIME, TIME, TIME, TIME, BOOLEAN, INTERVAL) IS
    'Does THIS employee_schedule row (dated p_row_shift_date) cover a booking on p_target_date? '
    'Same-date rows defer entirely to shift_covers_booking(). A row dated the day before only '
    'covers when it is a wrapping night shift reaching into p_target_date. Callers must widen '
    'their JOIN/EXISTS to `es.shift_date IN (p_target_date, p_target_date - 1)` — this function '
    'decides coverage per row, it does not select rows.';

-- The RPC below is byte-identical to 20260909210000 except the six shift-coverage
-- call sites now use shift_row_covers_booking() over a two-date JOIN instead of
-- shift_covers_booking() over a single-date JOIN. Recreated in full because
