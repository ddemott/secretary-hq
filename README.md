...
- 8 UI themes: navy (default), rose, forest, midnight, nord, sunset, high-contrast, solarized
...
- 9-step onboarding wizard (4-step solo variant): repeatable, re-enterable, with live coverage feedback and phone activation
...
- Appointment reminder scheduling via SMS and email (SMS is off, `ENABLE_SMS=false`, until 10DLC registration lands — email only until then)
...
| **Voice AI**  | Live — Telnyx → LiveKit Cloud → Deepgram Nova-3 (STT) + OpenAI GPT-4.1-mini (LLM) + Deepgram Aura (TTS; Grok/xAI TTS removed 2026-06-25). Call flow = question trees (`agent/src/checklist/`). Falling-tone spoken-prompt rewrites are in flight on [PR #527](https://github.com/ddemott/secretary-hq/pull/527) (`fix/voice-falling-tone-prompts`). PSTN inbound reaches the agent (confirmed 2026-06-30); the booking + transfer legs still need a live different-carrier call — see `docs/planning/TODO.md` (P0 Voice) + `docs/operations/RUNBOOK.md` §7. |
...
| **Tests**     | Root `npm test` green: 3,365 passing (281 files). Dashboard 1,242 (113 files) + agent 1,085 (63 files) green (2026-09-20). See `docs/planning/TEST_COVERAGE.md` for coverage detail.                                                                                                                                                                                                                                                                                                                                                                        |
...
│   ├── components/         170+ components (scheduler, CRM, settings, wizard)
...
| 9-step setup wizard    | Repeatable, re-enterable, live coverage feedback, phone activation (4-step solo mode) |
| 8 themes               | Navy (default), rose, forest, midnight, nord, sunset, high-contrast, solarized        |
